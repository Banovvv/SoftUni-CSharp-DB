    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                ...
                Console.WriteLine("To access the Hospital System provide your login information:");
                Console.Write("Enter your email: ");
                string email = Console.ReadLine();
                Console.Write("Enter your password: ");
                string password = Console.ReadLine();

                if (IsLoggedIn(context, email, password))
                {
                    Console.Clear();
                ...
                {
                    Console.WriteLine("Invalid email and/or password! Try again...");
                    Thread.Sleep(2000);
                }
            }
        }

        private static bool IsLoggedIn(HospitalDataContext context, string email, string password)
        {
            ILoginService service = new LoginService(context);

            return service.IsLoginSuccessfull(email, password);
        }
